...
<div class="container-fluid pt-4 px-4">
    <div class="bg-secondary rounded p-4">
        <div class="d-flex align-items-center justify-content-between mb-4">
            <h3 class="mb-0" style="color:red;">Ubah Laporan Keluhan</h4>
        </div>
        ...
        @endif
        <form action="{{ route('lapupdate', [$laporan->id]) }}" method="POST">@csrf
            ...
            <div class="form-floating mb-3">
                <textarea class="form-control @error('keluhan') is-invalid @enderror" name="keluhan" id="keluhan" rows="2" style="height:100%;" id="floatingInput" placeholder="name@example.com"
                    autocomplete="keluhan" autofocus>{{ $laporan->keluhan }}</textarea>
            ...
            <div class="form-floating mb-3">
                ...
                <input type="time" class="@error('kunjungan') is-invalid @enderror" name="kunjungan" value="{{ $laporan->kunjungan }}" autofocus>
                @error('kunjungan')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
                @enderror
                -
                <input type="time" class="@error('keluar') is-invalid @enderror" name="keluar" value="{{ $laporan->keluar }}" autofocus>
                @error('keluar')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
